...
Ported verbatim from ``readthedocs.config.exceptions``. Both classes inherit
from :class:`builder.exceptions.BuildUserError` so a config error reports as
a user-facing build error (with the appropriate ``message_id``).
"""
